...
bool isTrapped(int x, int y) {
    // Only check for traps when snake is long enough
    if (game.length < WIDTH * HEIGHT * 0.75) {
        return false;
    }
    ...
    // More lenient trap detection - only consider it trapped if very little space remains
    return reachable < game.length * 0.25;
}
...
float calculateReward(int prev_x, int prev_y, int x, int y, bool got_food, bool crashed) {
    ...
    float reward = (prev_dist - new_dist) * 10.0f;
    
    // Body avoidance (less penalizing than before)
    ...
    // Reward for maintaining safe distance from body
    float body_distance = calculateDistanceToBody(x, y);
    reward += body_distance * 2.0f;
    ...
    if (new_position) reward += 3.0f;
    
    // Small penalty for each step to encourage efficiency
    reward -= 0.5f;
    
    // Additional penalty if in a potential trap
    if (isTrapped(x, y)) {
        reward -= 10.0f;
    }
...
bool moveSnake(int& direction) {
    ...
    // Only end game on actual crashes or traps
    if (isTrapped(game.head_x, game.head_y)) {
        return true;
    }
